...
float4 calculatePointLight(int index, float3 worldPos, float3 normal)
{
    ...
    lightColour += pointlightAmbient[index];
...
float calcShadow(Texture2D sMap, float2 uv, float4 lightViewPosition, float bias)
{
    float shadowLevel = 0.0f;
    float SMAP_SIZE = 2048.f;
    float texStep = 1.0f / SMAP_SIZE;
    
    // Sample the shadow map (get depth of geometry)
    float depthValue = sMap.Sample(shadowSampler, uv).r;
	// Calculate the depth from the light.
    float lightDepthValue = lightViewPosition.z / lightViewPosition.w;
    float percentageWeight = 1.0f / 16.0f;
    lightDepthValue -= bias;
    float xValue = uv.x - (2 * texStep);
    float yvalue = uv.y - (2 * texStep);
    
    for (int y = 0; y < 4; y++)
    {
        for (int x = 0; x < 4; x++)
        {
            shadowLevel += sMap.Sample(shadowSampler, float2(xValue, yvalue)).r >= lightDepthValue ? percentageWeight : 0.0f;
            xValue += texStep;
        }
        xValue = uv.x - (2 * texStep);
        yvalue += texStep;
    }
...
float4 main(InputType input) : SV_TARGET
{
    ...
    float4 pointLightColour = { 0.0f, 0.0, 0.0f, 1.0f};
    for (int i = 0; i < 6; i++)
    {
    ...
    // Shadow test. Is or isn't in shadow
        ...
        float shadowIntensity = calcShadow(directionalDepthMap, pTexCoord, input.lightViewPos, shadowMapBias);
        directionalColour = (calculateLighting(-directionalDirection.xyz, input.normal, directionalDiffuse) * shadowIntensity);
    ...
    if (hasDepthData(pTexCoord2))
    {
        if (!isInShadow(spotDepthMaps[0], pTexCoord2, input.secondLightViewPos, shadowMapBias))
        {
            ...
            float shadowIntensity = calcShadow(spotDepthMaps[0], pTexCoord, input.secondLightViewPos, shadowMapBias);
    ...
    for (int numOfPointLights =  0; numOfPointLights < 1; numOfPointLights++)
    {
        for (int i = 0; i < 6; i++)
        {
            if (hasDepthData(pTexCoordsPoint[mapIndex]))
            {
                ...
                //{
                    float shadowIntensity = calcShadow(pointDepthMaps[mapIndex], pTexCoordsPoint[mapIndex], input.pointLightViewPositions[mapIndex], shadowMapBias);
                    pointLightColour = calculatePointLight(numOfPointLights, input.worldPosition, input.normal) * shadowIntensity;
